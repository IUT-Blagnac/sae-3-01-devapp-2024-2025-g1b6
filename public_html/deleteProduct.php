...
try {
    ...
    $queryCommandes = $pdo->prepare("
        ...
        FROM PANIER 
        WHERE IDPROD = ? AND IDCOMMANDE IS NOT NULL
    ");
    ...
    $queryPaniers = $pdo->prepare("SELECT COUNT(*) as nb FROM PANIER WHERE IDPROD = ? AND IDCOMMANDE IS NULL");
    ...
    $pdo->prepare("DELETE FROM PANIER WHERE IDPROD = ? AND IDCOMMANDE IS NULL")->execute([$idProd]);
    ...
    if (!empty($details)) {
        $message .= "Le produit a été retiré de : " . implode(", ", $details) . ".";
    }
