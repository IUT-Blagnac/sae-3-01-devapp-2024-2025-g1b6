...
async function loadMainCategories() {
    try {
        const response = await fetch('getCategories.php?with_public_cible=true');
        ...
        data.main_categories.forEach(category => {
            ...
            // Ajouter l'attribut de public cible
            option.setAttribute('data-public-cible', category.PUBLIC_CIBLE || '');
            
            selectPrincipal.appendChild(option);
        });

        // Ajouter un écouteur d'événement pour mettre à jour le public cible
        selectPrincipal.addEventListener('change', function() {
            const selectedOption = this.options[this.selectedIndex];
            const publicCibleSpan = document.querySelector('#publicCible span');
            const publicCible = selectedOption.getAttribute('data-public-cible') || 'Non défini';
            publicCibleSpan.textContent = publicCible;
        });
    } catch (error) {
