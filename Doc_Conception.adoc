...
- Un `Client` peut passer plusieurs `Commande`s.
- Chaque `Commande` est liée à un `Panier`.
- Un `Panier` peut contenir plusieurs `Produit`s.
- Un `Produit` appartient à une `Categorie` et une `SousCategorie`, et est associé à une `Marque`.
- Un `Produit` peut avoir plusieurs `Avis` laissés par des `Client`s.
- Un `Client` peut laisser plusieurs `Avis` sur des `Produit`s.
